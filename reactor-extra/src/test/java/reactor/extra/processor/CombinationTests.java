...
/*
	...
	public void mergeWithInterleave() {
		StepVerifier.create(Flux.merge(emitter1, emitter2))
		            .then(() -> emitValues())
	...
	public void mergeWithNoInterleave() throws Exception {
		Flux.concat(emitter1.log("test1"), emitter2.log("test2"))
		    .log()
	...
	public void sampleCombineLatestTest() throws Exception {
		...
		Flux.combineLatest(sensorOdd().cache()
	...
	public Flux<SensorData> sensorEven() {
		if (sensorEven == null) {
			...
			this.sensorEven = TopicProcessor.<SensorData>builder().name("even")
			                                                      .build();
	...
	public Flux<SensorData> sensorOdd() {
		if (sensorOdd == null) {
			...
			this.sensorOdd = TopicProcessor.<SensorData>builder().name("odd")
			                                                     .build();
