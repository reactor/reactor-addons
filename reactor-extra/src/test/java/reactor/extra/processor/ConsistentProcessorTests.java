...
public class ConsistentProcessorTests {
	...
	private void setupPipeline() {
		processor = TopicProcessor.<String>builder().autoCancel(false)
		                                            .build();
		workProcessor = WorkQueueProcessor.<String>builder().autoCancel(false)
		                                                    .build();
...
//				System.out.println("XXXX " + x);
	...
	class Receiver implements CoreSubscriber<String> {
		...
		public void onSubscribe(Subscription s) {
			Schedulers.single()
			          .schedule(() -> finish(s), 5000, TimeUnit.MILLISECONDS);
