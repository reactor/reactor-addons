...
final Action<Publisher<? extends String>, String> circuitSwitcher = Fluxion.switchOnNext(); // <3>
...
<5> Return a `Promise` from `Fluxion.toList()` to convert the `Stream` into an eventual `List`
