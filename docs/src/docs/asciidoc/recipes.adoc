...
Publisher<String> fileStream = new Publisher<String>() { // <1>
	...
	public void subscribe(final Subscriber<? super String> subscriber) {
		...
		try {
			...
			Flux.<String, FileNotFoundException> error(e)
				.subscribe(subscriber); // <8>
		}
	}
};

Flux.from(fileStream)
	.useCapacity(4L) // <9>
	.consumeOn( // <10>
...
Publisher<String> fileStream = Flux.generate(
	(n, sub) -> { // <1>
		String line;
...
We can use `Flux`, or <<streams.adoc#streams-basics, Streams factories>> like `Flux.generate()` to quickly achieve common use cases:
...
In this other exercise, we will focus more on the composition power in your hands with *Reactor Stream* module. A
...
[source,java]
----
final EmitterProcessor<String> closeCircuit = EmitterProcessor.create(); // <1>
final Flux<String> openCircuit = Flux.just("Alternative Message"); // <2>

final FluxProcessor<Publisher<? extends String>, String> circuitSwitcher = Flux.switchOnNext(); // <3>
		...
		circuitSwitcher // <6>
			...
			.doOnSubscribe(s -> { // <8>
				...
				if (failures.compareAndSet(maxErrors, 0)) {
					...
					Streams
						.timer(1)  // <10>
						.subscribe(ignore -> circuitSwitcher.onNext(closeCircuit));
				}
			})
			.retry() // <11>
			.toList()
...
----
<1> Create the main *hot* `EmitterProcessor` stream to send data later on.
...
<5> Return a `Promise` from `Flux.toList()` to convert the `Stream` into an eventual `List`
...
If you made it that far, here a simple non blocking stream to scale up your Konami codes over WebSocket. Tell me about a reward… Don't forget to add *Netty* to your classpath along *reactor-netty*.
...
keyboardFlux.onNext(KeyEvent.VK_RIGHT);
keyboardFlux.onNext(KeyEvent.VK_UP);
keyboardFlux.onNext(KeyEvent.VK_UP);
keyboardFlux.onNext(KeyEvent.VK_DOWN);
keyboardFlux.onNext(KeyEvent.VK_DOWN);
keyboardFlux.onNext(KeyEvent.VK_LEFT);
keyboardFlux.onNext(KeyEvent.VK_RIGHT);
keyboardFlux.onNext(KeyEvent.VK_LEFT);
keyboardFlux.onNext(KeyEvent.VK_RIGHT);
keyboardFlux.onNext(KeyEvent.VK_B);
keyboardFlux.onNext(KeyEvent.VK_A);
keyboardFlux.onNext(KeyEvent.VK_C);
keyboardFlux.onComplete();
